...
// - Added option to change found passwords
...
// Like the above function, but instead of copying the wrapping key into *out,
// copies the master key's salt into *out. - Brent
void dump_master_salt(char *out, char *master, char *buffer, size_t sz) {
    char *blob = get_blob(buffer, sz);
    memcpy(out, blob + 44, 20);
}
...
int main(int argc, char **argv) {
    ...
    for (i = 0; i < g_master_candidates_count; ++i) {
        ...
        if (key_len = dump_wrapping_key(key, g_master_candidates[i], buffer, sz)) {
            ...
            // testing
            char salt[20];
            dump_master_salt(salt, g_master_candidates[i], buffer, sz);
            char s_salt[20*2+1];
            hex_string(s_salt, salt, 20);
            printf("[ b( ^ .^)b ] Found master salt: %s\n", s_salt);
            // end testing
            break;
        }
    }
